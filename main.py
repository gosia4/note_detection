...
def detect_onsets_user_frequency(audio_data, threshold_factor=0.02, plot=True, fs=44100):
    # Oblicz spektrogram
    ...
    # Iteruj po kolumnach spektrogramu
    for i in range(1, len(t)):
        current_spectrum_values = []

        # Iteruj po wierszach spektrogramu
        for row in Sxx:
            current_spectrum_values.append(row[i])

        # Oblicz średnią wartość spektralną
        average_spectrum_value = np.mean(current_spectrum_values)

        # Ustaw próg dynamicznie
        threshold = threshold_factor * average_spectrum_value
        # threshold = threshold_factor * find_max_value(Sxx)


        # Sprawdź, czy aktualna wartość spektralna przekracza próg
    ...
    if onsets:
        ...
        for onset_time in onsets:
            i += 1
            print(i, f"{onset_time:.2f} seconds")
...
# Przykładowe nagranie dźwięku od użytkownika za pomocą pyaudio
...
# Wyświetl czasy onsetów dla danych od użytkownika na podstawie częstotliwości
...
# wav_file = wave.open('C:/Users/gosia/OneDrive/Pulpit/FTIMSET/wav/AndreHolzapfel/bongo1.wav', 'rb')  # treshold 30
frame_rate = wav_file.getframerate()
num_frames = wav_file.getnframes()
audio_data = np.frombuffer(wav_file.readframes(num_frames), dtype=np.int16)
wav_file.close()

# Wywołaj funkcję do detekcji onsetów
onsets = detect_onsets_user_frequency(audio_data, threshold_factor=80, plot=True, fs=frame_rate)
